...
# Main flow without signing
def process_apk(input_apk, temp_apk, final_apk):
    # Step 1: Zipalign
    zipalign(input_apk, temp_apk)

    # Step 2: Move unsigned APK to the final location (instead of signing)
    move_unsigned(temp_apk, final_apk)
...
def dcc_main(
    ...
    dex2c_file_data = dex2c_file_data.replace(
        'env->FindClass("np/loader/Protect");',
        'env->FindClass("' + custom_loader.replace(".", "/") + '");',
    )
    dex2c_file_data = dex2c_file_data.replace(
        "Java_dex2c_dcc_DccApplication", "Java_" + custom_loader.replace(".", "_")
    )
    ...
    if is_apk(apkfile) and outapk:
        ...
        filedata = filedata.replace(
            "Lnp/loader/Protect;", "L" + custom_loader.replace(".", "/") + ";"
        )
